...
class Unit: Decodable {
    ...
    func tilesInRange() -> HexArea {

        guard let range = self.unitType.properties?.range else {
            fatalError("can't get range")
        }

        return HexArea(center: self.position, radius: range)
    }

    func isWaitingForOrders() -> Bool {

        if let tacticalAI = self.tacticalAI {
            return tacticalAI.isWaitingForOrders()
        }
    ...
    func apply(damage: Int, suppression: Int, real: Bool) {
        ...
        if real {
            if damage > 0 {
                self.gameObject?.updateUnitStrengthIndicator()
            }

            if self.strength <= 0 {
                self.strength = 0

                self.gameObject?.removeFromParent()
            }
        }
    }
    ...
    func missionFinished(with result: MissionResult) {
        ...
        self.tacticalAI?.missionFinished(with: result)
    }

    func order(mission: Mission?) {

        self.tacticalAI?.order(mission: mission)
    }
}
