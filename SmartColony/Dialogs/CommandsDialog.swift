...
class CommandsDialog: Dialog {
    ...
    init(for commands: [Command]) {
        ...
        for command in commands {
            commandsDialogConfiguration.items.item.append(DialogItem(identifier: UUID.init().uuidString, type: .button, title: command.title(), fontSize: 18.0, result: command.type.dialogResult(), offsetx: 20, offsety: y + 10, anchorx: .center, anchory: .center, width: 100, height: 20, image: nil, selectedIndex: nil, items: nil))
            y += 25
        }
        
        commandsDialogConfiguration.items.item.append(DialogItem(identifier: UUID.init().uuidString, type: .button, title: "Cancel", fontSize: 18.0, result: .cancel, offsetx: 20, offsety: y + 10, anchorx: .center, anchory: .center, width: 100, height: 20, image: nil, selectedIndex: nil, items: nil))
