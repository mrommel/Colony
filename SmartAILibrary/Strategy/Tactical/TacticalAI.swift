...
public class TacticalAI: Codable {
    ...
    var currentDominanceZone: TacticalAnalysisMap.TacticalDominanceZone?
    ...
    func plotFreeformNavalOperationMoves(for operation: NavalOperation?, in gameModel: GameModel?) {
        ...
        if thisArmy.state == .waitingForUnitsToReinforce {
            ...
            if thisArmy.turnAtNextCheckpoint() == ArmyFormationSlot.unknownTurnAtCheckpoint {
                ...
                for index in 0..<thisArmy.numFormationEntries() {

                    // See if we are just able to get to muster point in time.  If so, time for us to head over there
                    if let slot = thisArmy.slot(at: index), let unit = slot.unit {

                        if !unit.processedInTurn() {

                            let thisSlotEntry = thisArmy.formation.slots()[index]
    ...
    func plotNavalEscortOperationMoves(for operation: NavalEscortedOperation?, in gameModel: GameModel?) {
        ...
        if thisArmy.state == .waitingForUnitsToReinforce {
            ...
            if civilian != nil && !civilianTile!.isWater() && civilian!.location == thisArmy.goal {
                ...
                for index in 2..<thisArmy.numFormationEntries() {
            ...
            var closestUnitAtSea: AbstractUnit?
            var bestPlotTmp: HexPoint?
                ...
                for index in 0..<thisArmy.numFormationEntries() {
                    ...
                    if let slot = thisArmy.slot(at: index), let unit = slot.unit {
                        ...
                        if !unit.processedInTurn() {
                            ...
                            let slotEntry = thisArmy.formation.slots()[index]
    ...
    func executeNavalFormationMoves(of army: Army?, towards turnTarget: HexPoint, in gameModel: GameModel?) {
        ...
        if self.operationUnits.isEmpty {
            return
        }
            ...
            if let plot = gameModel.tile(at: plotPoint) {
                ...
                if plotDistance <= range {
                    ...
                    if operation.isMixedLandNavalOperation() && cell.canUseForOperationGatheringCheckWater(isWater: true) {
                        ...
                        valid = true
                    } else if (!operation.isAllNavalOperation() && !operation.isMixedLandNavalOperation()) &&
                                (cell.canUseForOperationGatheringCheckWater(isWater: false) || false /* GC.getMap().GetAIMapHint() & 1*/) {
                    ...
                    if plotDistance <= range {
                        ...
                        if operation.isMixedLandNavalOperation() && cell.canUseForOperationGatheringCheckWater(isWater: true) {
                            ...
                            valid = true
                                    ...
                                    (cell.canUseForOperationGatheringCheckWater(isWater: false) || gameModel.isPrimarilyNaval()) {
                            valid = true
                            if cell.isWater() {
                                forcedToUseWater = true
                            }
    ...
    func performChosenMoves(fallbackMoveRange: Int = 1, in gameModel: GameModel?) {
        ...
        for chosenBlockRef in self.chosenBlocks {
            ...
            if let unit = chosenBlock.unit {
                ...
                if unit.location != chosenBlock.point && self.isInChosenMoves(at: unit.location) &&
                    gameModel.numFriendlyUnits(
                        at: chosenBlock.point,
                        player: self.player,
                        of: unit.unitMapType()) == 0 {
    ...
    func executeSafeBombards(on target: TacticalTarget, in gameModel: GameModel?) -> Bool {
        ...
        var firstAttackerCity: AbstractCity?
        var firstAttackerUnit: AbstractUnit?
                        ...
                        if cell.isRevealed() && cell.canUseForOperationGathering() {
                            ...
                            if cell.isWithinRangeOfTarget() && !cell.isSubjectToAttack() {
                                ...
                                if self.findClosestUnit(
                                    ...
                                    needsIgnoreLineOfSight: !haveLightOfSight,
        ...
        for zoneTarget in self.zoneTargets {
            ...
            if type == .none {
                ...
                if zoneTarget.targetType == .highPriorityUnit || zoneTarget.targetType == .mediumPriorityUnit ||
                    zoneTarget.targetType == .lowPriorityUnit || zoneTarget.targetType == .city ||
                    zoneTarget.targetType == .improvement {
    ...
    func plotCloseOnTarget(checkDominance: Bool, in gameModel: GameModel?) {
        ...
        if zone.territoryType == .tempZone {
            ...
            let target = TacticalTarget(targetType: .barbarianCamp, target: zone.center!.point, targetLeader: .barbar, dominanceZone: zone)
            self.executeCloseOnTarget(target: target, zone: zone, in: gameModel)
                ...
                for zoneTarget in self.zoneTargets {
                    ...
                    if zoneTarget.targetType == .highPriorityUnit ||
                        ...
                        if zoneTarget.isTargetStillAlive(for: self.player, in: gameModel) {
                            ...
                            gameModel.tacticalAnalysisMap().clearDynamicFlags()
                            gameModel.tacticalAnalysisMap().setTargetBombardCells(
    ...
    func executeOneProtectedBombard(on target: TacticalTarget, in gameModel: GameModel?) -> Bool {
        ...
        var firstAttacker: AbstractUnit?
                        ...
                        if cell.isWithinRangeOfTarget() {
                            ...
                            if noEnemyAdjacent {
                                ...
                                if self.findClosestUnit(
                                    ...
                                    needsIgnoreLineOfSight: !haveLineOfSight,
                                    mustBeMeleeUnit: false,
                                    ignoreUnits: false,
                                    rangedAttackTarget: targetPlot,
                                    in: gameModel) {
        ...
        if blocksToCreate > 0 && blocksToCreate < 6 {
            ...
            while !done {
                ...
                for index in 0..<blocksToCreate {
                    ...
                    if index >= self.potentialBlocks.count {
            ...
            for index in 0..<self.newlyChosen.count {
    ...
    func scoreAssignments(canLeaveOpenings: Bool) -> Int {
        ...
        for index in 0..<self.temporaryBlocks.count {
            ...
            for index2 in (index + 1)..<self.temporaryBlocks.count {

                guard let unit1 = self.temporaryBlocks[index]?.unit else {
                    continue
                }

                if unit1.isEqual(to: self.temporaryBlocks[index2]?.unit) {
                    ...
                    if !canLeaveOpenings {
                        ...
                        if self.temporaryBlocks[index]!.distanceToTarget < self.temporaryBlocks[index2]!.distanceToTarget {
                            self.temporaryBlocks[index2]?.distanceToTarget = Int.max
                        } else {
                            self.temporaryBlocks[index]?.distanceToTarget = Int.max
                        }
                    }
                }
            }
    ...
    func numUniqueUnitsLeft() -> Int {
        ...
        for index in 1..<self.temporaryBlocks.count {

            if !self.temporaryBlocks[index]!.unit!.isEqual(to: current) {
                rtnValue += 1
    ...
    func haveDuplicateUnit() -> Bool {
        ...
        for index in 0..<(self.newlyChosen.count - 1) {

            for index2 in (index + 1)..<self.newlyChosen.count {

                if self.newlyChosen[index]!.unit!.isEqual(to: self.newlyChosen[index2]!.unit) {
                    return true
                }
            }
        }
    ...
    func removeChosenUnits(startIndex: Int) {
        ...
        for index in startIndex..<self.temporaryBlocks.count {

            var copyIt = true

            guard let blockingUnit = self.temporaryBlocks[index] else {
                continue
            }

            // Loop through chosen array looking for occurrences of this unit
            for index2 in 0..<self.newlyChosen.count {

                if !copyIt {
                    continue
                }

                if blockingUnit.unit!.isEqual(to: self.newlyChosen[index2]?.unit) {
                    copyIt = false
                }
            }
    ...
    private func identifyPriorityTargets(in gameModel: GameModel?) {
        ...
        for cityRef in gameModel.cities(of: player) {
            ...
            if let city = cityRef {
                ...
                else if !possibleAttackers.isEmpty {
