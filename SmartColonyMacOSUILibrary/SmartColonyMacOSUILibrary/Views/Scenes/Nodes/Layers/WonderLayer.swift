...
class WonderLayer: BaseLayer {
    ...
    func placeTileHex(for tile: AbstractTile, and point: HexPoint, at position: CGPoint, alpha: CGFloat) {
        ...
        if wonder != .none {
            ...
            self.textureUtils?.set(wonderSprite: wonderSprite, at: point)
            ...
            self.textureUtils?.set(wonderBuildingSprite: wonderBuildingSprite, at: point)
        }
    }
    ...
    override func clear(at point: HexPoint) {
        ...
        if let wonderSprite = self.textureUtils?.wonderSprite(at: point) {
            self.removeChildren(in: [wonderSprite])
        }

        if let wonderBuildingSprite = self.textureUtils?.wonderBuildingSprite(at: point) {
            self.removeChildren(in: [wonderBuildingSprite])
        }

        if let wonderSprite = self.textureUtils?.wonderSprite(at: alternatePoint) {
            self.removeChildren(in: [wonderSprite])
        }

        if let wonderBuildingSprite = self.textureUtils?.wonderBuildingSprite(at: alternatePoint) {
            self.removeChildren(in: [wonderBuildingSprite])
        }
    }
